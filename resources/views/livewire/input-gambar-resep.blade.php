            ...
            @foreach ($images as $index => $image)
                @if (!empty($image))
                    <div class="w-40 h-40 rounded-2xl">
                        <img class="object-cover h-full w-full rounded-2xl" src="{{ asset('/storage/'.$image) }}" alt="gambar-resep">
                ...
                <div
                    ...
                    <div class="absolute">
                        <div class="flex flex-col items-center>
                            <span class=" block
                            text-center text-gray-400 font-normal">Tambah Gambar atau Foto</span>
                        </div>
                    </div>
                    <input wire:model="tempImage" type="file" class="h-full w-full cursor-pointer opacity-0"
                        name="gambar">
                </div>
            </div>
        </div>
